...
def generate_summary_and_extract_tables(text):
    ...
    for chunk in chunks:
        response = client.chat.completions.create(
            messages=[{"role": "user",
                       "content": f"Summarize the introduction paragraph of the given input pdf and focus on extracting numerical data. "
                                  f"Avoid unnecessary commentary. "
                                  f"Here is the input:\n\n{chunk}"}],
        ...
        tables = re.findall(r'(\d+[\d\s,]*[\n\t\s]*\d+.*?(?:\n\s*\n|$))', content, re.DOTALL)
        extracted_tables.extend([t for t in tables if t.strip()])
