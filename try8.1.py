        ...
        response = client.chat.completions.create(
            messages=[{"role": "user",
                       "content": f"Summarize the Introduction part of the input data."
                                  f"Extract tables from whole input data if present somewhere with numerical data."
                                  f"While reading the input textual data extract the numerical data from it where you find its context through text also create tables by it, while omitting non-numerical information."
...
if __name__ == '__main__':
    ...
    generate_poster("template1.html", "poster.html", title, summary_text, plots, tables)
