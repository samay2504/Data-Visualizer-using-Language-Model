...
# Updated poster creation function
def create_poster(text, bullet_points, charts, output_path):

    poster_width = 1600  # Increased width
    poster_height = 1800  # Increased height
    ...
    title_font_size = 50  # Adjusted font sizes
    ...
    text_max_width = poster_width // 2 - 40  # Adjusted to fit in the left half of the poster
    ...
    chart_x_offset = poster_width // 2 + 20  # Right half of the poster for charts
    chart_y_offset = 80  # Start at the top of the right half

    for i, chart_path in enumerate(charts):
        chart = Image.open(chart_path)
        chart.thumbnail((poster_width // 2 - 40, 300))  # Adjust thumbnail size for plots
        poster.paste(chart, (chart_x_offset, chart_y_offset))
        chart_y_offset += chart.size[1] + 10

        chart_title = f"Chart {i + 1}: Summary of Extracted Data"
        draw.text((chart_x_offset, chart_y_offset), chart_title, font=chart_title_font, fill=title_color)
